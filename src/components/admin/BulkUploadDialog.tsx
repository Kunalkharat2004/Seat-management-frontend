...
const BulkUploadDialog = ({ open, onClose }: BulkUploadDialogProps) => {
    ...
    const uploadMutation = useMutation({
        mutationFn: (file: File) => bulkUploadEmployees(file),
        onSuccess: (data) => {
            ...
            if (data.successful_creations > 0) {
                parts.push(`${data.successful_creations} employee${data.successful_creations === 1 ? "" : "s"} created`);
            }
    ...
    const handleUpload = () => {
        ...
        // Re-validate before calling API (belt & suspenders)
                    ...
                    <Box className="flex flex-col gap-4 pt-1">
                        ...
                        {result && (
                            ...
                            >
                                ...
                                {/* Error detail — shown when there are failures */}
                                {result.failed_rows > 0 && (
                                    ...
                                    >
                                        ...
                                        failed to import. This usually means
                                        missing required fields (employee_id,
                                        name, or email). Check the CSV and
